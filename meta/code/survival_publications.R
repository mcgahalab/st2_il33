...
dir.create(file.path(PDIR, proj, "cibersort"), showWarnings = F)
outf <- file.path(PDIR, proj, "cibersort", paste0(proj, ".cibersort.rds"))
if(!file.exists(outf)){
  file.create(outf)
  cibersort <- deconvolute(tpm, "cibersort")
  saveRDS(cibersort, file=outf)
} else {
  cibersort <- tryCatch({readRDS(outf)}, error=function(e){NULL})
}

cibersort_mat <- as.data.frame(cibersort) %>% 
  dplyr::rename_with(., ~make.unique(gsub("^.*?\\.", "", .))) %>%
  tibble::column_to_rownames(., "cell_type") 


cibersort_cor <- apply(ssgsea_dat, 1, function(i){
  int_ids <- purrr::reduce(list(names(i), colnames(cibersort_mat)), .f=intersect)
...
genesets <- c('Treg_Sig')
cor_df <- lapply(genesets, function(geneset_j){
  data.frame("geneset"=geneset_j, 
             "proj"=proj,
             "r"=sapply(cibersort_cor[[geneset_j]], function(x) x$estimate),
             "p"=sapply(cibersort_cor[[geneset_j]], function(x) x$p.value)) %>%
    mutate("log10padj"=-1*log10(p.adjust(p, method='BH')),
           "sig" = case_when(r >= 0.2 & log10padj >= (-1*log10(0.05)) ~ "up",
                             r <= -0.2 & log10padj >= (-1*log10(0.05)) ~ "down",
                             TRUE ~ "ns")) 
}) %>% do.call(rbind, .) %>%
...
pdf(file.path(PDIR, proj, "cibersort", paste0(proj, ".cibersort.pdf")), width = 5, height = 5)
final_plot; dev.off()
file.copy(file.path(PDIR, proj, "cibersort", paste0(proj, ".cibersort.pdf")),
          to="~/xfer", overwrite = T)
message(paste0("xfer ", proj, ".cibersort.pdf\n"))
...
cibersort_cor_l <- lapply(ssgsea_l, function(ssgsea_dat){
...
genesets <- c('Treg_Sig')
cor_df <- lapply(names(cibersort_cor_l), function(corid){
  cibersort_cor <- cibersort_cor_l[[corid]]
...
final_plot <- ggplot(cor_df, aes(x=r, y=log10padj)) +
  ...
  geom_point(data = sig_down,
             ...
             fill = "steelblue", 
             colour = "black") + 
  # geom_point(data = nonsig_up,
  #            shape = 21,
  #            size = 4, 
  #            fill = "indianred1", 
  #            colour = "black") + 
  # geom_point(data = nonsig_down,
  #            shape = 21,
  #            size = 4, 
  #            fill = "lightsteelblue", 
  #            colour = "black") +
  facet_grid(geneset ~ corid, space='free') +
  geom_hline(yintercept = -log10(0.05),
             linetype = "dashed") + 
  geom_vline(xintercept = c(-0.2, 0.2),
             linetype = "dashed") +
  ggrepel::geom_label_repel(data = rbind(sig_up, sig_down),   
                            ...
                            size=3,
                            force = 2) +
  # ggrepel::geom_label_repel(data = rbind(nonsig_up, nonsig_down),   
  #                           aes(label = celltype),
  #                           size=3,
  #                           force = 2) +
  scale_colour_manual(values = cols) + 
...
pdf(file.path(PDIR, proj, "cibersort", paste0(proj, ".cibersort.pdf")), width = 17, height = 5)
final_plot; dev.off()
file.copy(file.path(PDIR, proj, "cibersort", paste0(proj, ".cibersort.pdf")),
          to="~/xfer", overwrite = T)
message(paste0("xfer ", proj, ".cibersort.pdf\n"))
...
ssgsea_l <- lapply(fpkm_l, function(fpkm){
...
if(!file.exists(outf)){
  file.create(outf)
  cibersort <- deconvolute(fpkm_l$on, "cibersort")
  saveRDS(cibersort, file=outf)
} else {
  cibersort <- tryCatch({readRDS(outf)}, error=function(e){NULL})
}
...
cibersort_cor <- apply(ssgsea_l$on, 1, function(i){
  int_ids <- purrr::reduce(list(names(i), colnames(cibersort_mat)), .f=intersect)
  if(length(int_ids)==0) return(NULL)
  apply(cibersort_mat[,int_ids], 1, function(j){
    cor.res <- cor.test(i[int_ids], as.numeric(j))
