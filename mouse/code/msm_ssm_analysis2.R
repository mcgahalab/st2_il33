...
res_cispbs <- lapply(reflvls, function(reflvl_i){
  ...
  dds <- .getDds(cts, meta, formula='~condition')
  
  coef <- grep(gsub("-", ".", paste0("_", altlvl_i)), resultsNames(dds), value=T)
  overall_reslfc_ln <- lfcShrink(dds, coef=coef, type="apeglm") %>%
    as.data.frame() %>%
    tibble::rownames_to_column(var='ens') %>%
...
### Compare the change of [CIS vs PBS] between [TDLN vs LN] for each Macrophage
meta$macro_ln <- factor(with(meta, paste0(celltype, "_", lnstatus)))
meta$treatment <- factor(meta$treatment)
meta$treatment <- relevel(meta$treatment, ref='PBS')
reflvls <- grep("_LN", levels(meta$macro_ln), value=T)
res_int <- lapply(reflvls, function(reflvl_i){
  print(reflvl_i)
  altlvl_i <- gsub("LN", "TDLN", reflvl_i)
  meta$macro_ln <- relevel(meta$macro_ln, ref=reflvl_i)
  dds <- .getDds(cts, meta, formula='~macro_ln+treatment+macro_ln:treatment')
...
res_all_macrophage <- c(res_cispbs, res_int)
gg_volcanos <- lapply(names(res_all_macrophage), function(resid){
  deg_res <- res_all_macrophage[[resid]]
  ...
  if(grepl("condition", uid)){
    ...
    subtitle <- gsub("macro_ln|treatment", "", uid) %>%
      paste0(., " vs ", gsub("_TDLN", "_LN", .) %>%
               gsub("CIS", "PBS", .))
  }
...
pdf(file.path(pdir, "diffexp", "volcano_plots_all.filtered.pdf"),
    width = 12, height = 11)
cowplot::plot_grid(plotlist=gg_volcanos[1:4], ncol=2,  nrow = 2)
cowplot::plot_grid(plotlist=gg_volcanos[5:6], ncol=2, nrow = 2)
dev.off()
file.copy(file.path(pdir, "diffexp", "volcano_plots_all.filtered.pdf"),
          to="~/xfer", overwrite = T)

saveRDS(res_all, file=file.path(pdir, "diffexp", "res_all.rds"))
...
genes_to_include <- read.table(file.path(pdir, "..", "ref", "msm_ssm_gsea_genes", "msm_ssm.txt"),
  ...
  dplyr::filter(gm$ENSEMBL$gene_biotype[V1] == 'protein_coding') %>% 
...
ct_res_gsea <- lapply(lfcs, function(lfc_cols){
  ...
  ct_res_gsea <- lapply(celltypes, function(celltype){
    ...
    if(filter_genes) res <- res %>% dplyr::filter(ens %in% genes_to_include$V1)
    
    # Run GSEA using the LFC for the interaction and the CIS-treatment terms
    gsea <- lapply(lfc_cols, function(lfc_col){
      # pval_col <-gsub("log2FoldChange", "pvalue", lfc_col)
      # lfc_v <- setNames((sign(res[,lfc_col]) * -log10(res[,pval_col]+0.001)),
      #                   ens2entrez_ids[res$ens])
      lfc_v <- setNames(res[,lfc_col],
                        ens2entrez_ids[res$ens])
                        # gm$ENSEMBL$ENTREZ[res$ens])
      msig_ds <- msigdbr(species = 'Mus musculus', category = 'C2', subcategory = "CP:REACTOME") %>%
        dplyr::select(gs_name, entrez_gene) %>%
        as.data.frame()
      gse <- GSEA(sort(na.omit(lfc_v), decreasing = T), TERM2GENE = msig_ds, pvalueCutoff = 1)
      
      gsea_v <- iterateMsigdb(species='Mus musculus', msig_lvls=msig_lvls[2], 
...
# ct_res_gsea_bkup <- readRDS(file.path(outdir, "GSEA-response.selectedGenes.2cyto.rds")) # Dont use
...
for(celltype in names(ct_res_gsea_bkup)){
  ...
  coldata_i <- coldata %>% dplyr::filter(celltype==!!celltype)
  tpm_i <- tpm %>% 
    dplyr::select(rownames(coldata_i)) %>%
    tibble::rownames_to_column("ens") %>% 
    mutate(GeneName=gm$ENSEMBL$SYMBOL[ens],
           Description=ens2biotype_ids[ens]) %>% 
    dplyr::filter(Description=='protein_coding') %>% 
    dplyr::select(-c(ens)) %>% 
